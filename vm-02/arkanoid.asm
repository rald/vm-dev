...
draw_score:
    ; --- Extract Decimal Digits (Tens and Units) from score ---
    LDR R1, score       ; R1 = score
    MOV R2, 0h          ; R2 = tens counter

score_div_loop:
    MOV R3, R1
    SUB R3, 0Ah         ; R3 = score - 10
    MOV R4, R3
    AND R4, 8000h       ; Check sign bit of (score - 10)
    CMP R4, 8000h
    JNZ score_sub
    JMP score_div_done
...
score_div_done:
    ; R2 = tens digit, R1 = units digit (remainder)

    ; --- Render Tens Digit at X = 4h, Y = 4h ---
    MOV R3, R2
    SHL R3, 4h          ; digit * 16 bytes (8 words * 2 bytes)
    MOV R4, font_digits
    ADD R3, R4          ; R3 = font pointer for tens digit
...
tens_row_loop:
    ...
    MOV R9, R6
    SHL R9, 1h          ; row * 2 bytes per word
    MOV RA, R3
    ADD RA, R9
    LDR RB, RA          ; RB = 16-bit row bitmap

    MOV R0, 8h          ; col = 8 pixels (Using R0 instead of reserved RC)
tens_col_loop:
    MOV RA, RB          ; Use RA as temp for bit test
    AND RA, 0080h       ; Test MSB (bit 7)
    CMP RA, 0080h
    JNZ tens_skip_px
    STR R7, R5
tens_skip_px:
    ...
    ; --- Render Units Digit at X = 0ch (12), Y = 4h ---
    MOV R3, R1          ; R1 holds units digit remainder
    SHL R3, 4h          ; digit * 16 bytes
    MOV R4, font_digits
    ADD R3, R4          ; R3 = font pointer for units digit
...
; =====================================================================
; Variable Declarations in RAM & Font Bitmaps (0 to 9)
; =====================================================================
.ORG 7000h
paddle_x:  .DW 0h
ball_x:    .DW 0h
...
font_digits:
; Digit 0
.DW 003Ch, 0042h, 0042h, 0042h, 0042h, 0042h, 0042h, 003Ch
; Digit 1
.DW 0018h, 0038h, 0018h, 0018h, 0018h, 0018h, 0018h, 007Eh
; Digit 2
.DW 003Ch, 0042h, 0002h, 001Ch, 0020h, 0040h, 0040h, 007Eh
; Digit 3
.DW 003Ch, 0042h, 0002h, 001Ch, 0002h, 0002h, 0042h, 003Ch
; Digit 4
.DW 0008h, 0018h, 0028h, 0048h, 007Eh, 0008h, 0008h, 0008h
; Digit 5
.DW 007Eh, 0040h, 0040h, 007Ch, 0002h, 0002h, 0042h, 003Ch
; Digit 6
.DW 001Ch, 0020h, 0040h, 007Ch, 0042h, 0042h, 0042h, 003Ch
; Digit 7
.DW 007Eh, 0042h, 0004h, 0008h, 0010h, 0010h, 0010h, 0010h
; Digit 8
.DW 003Ch, 0042h, 0042h, 003Ch, 0042h, 0042h, 0042h, 003Ch
; Digit 9
.DW 003Ch, 0042h, 0042h, 003Eh, 0002h, 0002h, 0042h, 003Ch