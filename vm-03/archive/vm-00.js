...
const FB_START = 0x8000;         //[cite: 3] Framebuffer start address
const FB_SIZE = 128 * 128;       //[cite: 3] Framebuffer size (16,384 bytes)
const STACK_START = 0xF000;      //[cite: 3] Stack start pointer
const CALL_STACK_START = 0xF400; //[cite: 3] Call stack start pointer
const TIMESTAMP_ADDR = 0xFFF0;   //[cite: 3] Timestamp MMIO (8 bytes)
const MOUSE_X_ADDR = 0xFFF8;     // Mouse X coordinate MMIO (1 byte)
const MOUSE_Y_ADDR = 0xFFF9;     // Mouse Y coordinate MMIO (1 byte)
const KEYBOARD_ADDR = 0xFFFC;    //[cite: 3] Keyboard MMIO (1 byte)
...
function resetVM() {
    ...
    regs[RC] = 0x0100; //[cite: 3] Program Counter reset address
    console.log("[Pantasya VM] VM Reset. Registers initialized:", Array.from(regs).map(r => `0x${r.toString(16)}`));
}
    ...
    for (let pl of pendingLines) {
        ...
        let getVal = (arg, isRegExpected = false) => {
            ...
            if (arg.startsWith('R') || arg.startsWith('r')) {
                ...
                if (isNaN(regIdx) || regIdx < 0 || regIdx > 15) {
                    throw new Error(`Line ${pl.lineNum}: Invalid register name "${arg}"`);
                }
...
window.addEventListener('keydown', (e) => {
    ...
    console.log(`[Peripherals] Key Down: keyCode = ${e.keyCode}`);
});
